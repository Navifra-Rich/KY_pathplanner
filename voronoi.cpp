...
vector<VEdge> Voronoi::ComputeVoronoiGraph(vector<Pos*> p, double minY, double maxY)
{
	...
	qsort(sites, nsites, sizeof(*sites), PosCompare);
...
Halfedge *  Voronoi::ELleftbnd(Pos *p)
{
	int i, bucket;
	Halfedge *he;
...
Site * Voronoi::intersect(Halfedge *el1, Halfedge *el2, Pos *p)
{
	Edge *e1, *e2, *e;
	Halfedge *el;
	double d, xint, yint;
...
int Voronoi::right_of(Halfedge *el, Pos *p)
{
	Edge *e;
	Site *topsite;
